...
- Fix: ABAP dead candidates now cover any Z/Y-prefixed class, not only those following the `ZCL_`/`YCL_` convention. Classes named `ZORDER`, `ZCALCULATOR`, etc. were previously excluded from dead code detection. Corrected by removing the `CL_` suffix requirement from `_Z_CLASS_RE` and `_Z_METHOD_RE` in `build.py`.
...
- Feat: SAP transaction extractor — `extract_tran()` parses abapGit `.tran.xml` exports (`TSTC`/`TSTCT` elements) and emits one `abap_tran` node per TCODE with `tcode`, `pgmna`, `dypno`, and `ttext` attributes, plus a `launches EXTRACTED` edge to the target program stub (Z/Y-prefixed only). Program stubs use the same `abap_prog_*` global ID scheme as `extract_abap`, so they reconcile automatically when the `.abap` source is also in the corpus. Programs reached by at least one transaction are never marked dead; programs with no callers anywhere receive `dev_tool: true` instead of `dead_candidate: true`.
- Feat: ABAP function module and SUBMIT extraction — `extract_abap` now handles `function_definition` (emits `FUNCTION <name>` nodes with global `abap_fn_*` IDs and uses the FM as scope owner for inner `CALL FUNCTION` edges), `function_pool_statement` (emits `FUNCTION GROUP <name>` nodes with `abap_fg_*` IDs from the TOP include), and `submit_statement` (emits `submits EXTRACTED` edges from the calling program to the target program stub, Z/Y-prefixed only; dynamic `SUBMIT (var)` silently skipped). Function modules excluded from dead-candidate marking: they may be called from SAP-standard enhancement frameworks (CMOD/SMOD) outside the corpus.
- Feat: ABAP OO events and BADI extraction — `extract_abap` now handles `events_declaration` / `class_events_declaration` (emits `EVENT <name>` nodes scoped to the declaring class, kind inherited from the class), `raise_event_statement` (emits `raises EXTRACTED` edges), `set_handler_statement` (emits `uses INFERRED` edges when subject is `ME`; other objects not resolvable statically), `get_badi_statement` (emits `uses INFERRED` edges to the BADI interface via the `TYPE` field), and `call_badi_statement` (emits `calls INFERRED` edges to an `abap_badi_method` stub; target class resolved at runtime via BADI registry, out of scope for static analysis).
- Fix: `extract_tran` now parses real abapGit `.tran.xml` exports correctly. The previous wrapping strategy (`<root>…</root>`) failed when the file contained an `<?xml?>` declaration (invalid XML inside an element). Fix: declaration stripped with a non-greedy regex before wrapping; namespace-agnostic XPath `.//{*}TSTC` handles `asx:` prefixed structures. Existing bare-element fixtures continue to work.
- Fix: ABAP method nodes no longer generate 1 030 false-positive `dead_candidate` results. Method definition IDs used `_make_id(file_stem, class, method)` while cross-file call-site stubs used a different stem → IDs never reconciled → in-degree always 0. IDs now follow a global scheme `_make_id("abap_method", class, method)` shared by definitions, `SET HANDLER` stubs, and static `=>` call stubs. Static `=>` call stubs also switched from `_ensure` to `_ensure_stub` to avoid polluting definition nodes with call-site source locations.
- Fix: ABAP `method_call` handler now correctly emits `calls INFERRED` edges for both static (`CLASS=>METHOD`) and instance (`var->METHOD`) calls. The previous implementation called `child_by_field_name("source")` which returns only the first `source`-tagged child (the identifier), never the operator (`=>` or `->`), so the operator check always failed and no edges were produced. Fix: iterate all children with `field_name_for_child()` to detect the operator by child type. A pre-pass over `data_spec` nodes builds a `var_type: dict[str, str]` map (variable → class) for resolving `lo_var->METHOD()` calls; unresolved variables (no `TYPE REF TO` declaration found, or interface-typed) are silently skipped.
- Fix: static `ZCL=>METHOD()` calls now also emit a `uses INFERRED` edge to the `abap_cls_*` node in addition to the `calls INFERRED` edge to the method stub. Without this edge `mark_dead_candidates()` saw zero cross-file in-degree on the class node and incorrectly flagged classes used only via static method calls as `dead_candidate: true` (confirmed example: `ZCL_SELECTION_SCREEN`).
- Fix: `RAISE EXCEPTION TYPE zcx_xxx` and `RAISE EXCEPTION TYPE zcx_xxx EXPORTING ...` now emit a `uses INFERRED` edge to the exception class node. Previously no edge was emitted for class-based exception raising, causing every Z/Y exception class used only via `RAISE EXCEPTION TYPE` to appear as `dead_candidate: true` (false positives; confirmed: `ZCX_VALE_ERROR`, `ZCX_IDOC_VALE`). `RAISE EXCEPTION NEW zcx_xxx()` was already covered by the existing `new_expression` handler.
- Feat: `mark_dead_candidates()` now accepts an optional `corpus_root` parameter and loads `<corpus_root>/.graphify_known_alive` when present. Objects listed in that file (one per line, case-insensitive, `#` comments ignored) are never marked `dead_candidate` regardless of cross-file in-degree. Intended for objects called from code outside the corpus: Smartforms auto-generated function groups, RFC callers from external systems, CMOD/SMOD exits, etc.
- Fix: Windows path separator in `_queue_pending` (`watch.py`) — replaced `os.fspath()` with `Path.as_posix()` so pending-changes files always use forward slashes, matching the expectations of the drain reader.
- Fix: Windows path separator in `llm._build_image_refs` and `_build_context_parts` — replaced `str(p.relative_to(root))` with `p.relative_to(root).as_posix()` so `rel` fields in image references use forward slashes on Windows.
- Fix: `skillgen/gen._git_show` now passes `encoding="utf-8"` to `subprocess.run` to avoid `UnicodeDecodeError` on Windows systems where the default encoding is `cp1252`.
